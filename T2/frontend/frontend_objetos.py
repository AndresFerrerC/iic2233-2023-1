...
from parametros import PATH_PERSONAJES, IMAGENES_FANTASMA_VERTICAL, \
    IMAGENES_FANTASMA_HORIZONTAL, ICONOS_BLOQUES, PATH_BLOQUES, \
    INTERVALO_SPRITES, DIMENSION_SPRITES, IMAGENES_FANTASMA_FOLLOWERVILLAIN
import os


# Fantasmas
class FantasmaHorizontal(QLabel):
    ...
    def instalar_personaje(self) -> None:
        ...
        self.pixmap_principal = self.sprites_animaciones["right"][0]  # Inicial
    ...
    def instanciar_imagenes(self) -> None:
        ...
        for tipo_movimiento in IMAGENES_FANTASMA_HORIZONTAL:
            lista_qpixmap = []
            for imagen in IMAGENES_FANTASMA_HORIZONTAL[tipo_movimiento]:
    ...
    def animar(self) -> None:
        ...
        if self.tipo_movimiento == "left":
            lista = self.lista_imagenes_izquierda
        else:  # derecha o estático
    ...
    def detectar_tipo_movimiento(self, posicion_mapa: tuple) -> str:
        """Retorna el tipo de movimiento (left, right)
        dependiendo de las nuevas posiciones dentro del mapa"""
        posicion_x = self.x()
        nueva_x = posicion_mapa[0]
        if nueva_x > posicion_x:
            return "right"
...
class FantasmaVertical(QLabel):
    ...
    def instalar_personaje(self) -> None:
        """Instala el personaje en las coordenadas iniciales (x, y)"""
        self.instanciar_imagenes()
        self.pixmap_principal = self.sprites_animaciones[0]  # Inicial
        self.setPixmap(self.pixmap_principal)
        self.setGeometry(*self.posicion_inicial, *DIMENSION_SPRITES)

    def instanciar_imagenes(self) -> None:
        """Instancia una lista conteniendo referencias
        a los QPixmap de cada imagen del personaje"""
        self.sprites_animaciones = []
        for imagen in IMAGENES_FANTASMA_VERTICAL:
            path_imagen = os.path.join(*PATH_PERSONAJES, imagen)
            pixmap_imagen = QPixmap(path_imagen)
            self.sprites_animaciones.append(pixmap_imagen)

    def detectar_tipo_movimiento(self, posicion_mapa: tuple) -> str:
        """Retorna el tipo de movimiento (left, right)
        dependiendo de las nuevas posiciones dentro del mapa"""
        posicion_y = self.y()
        nueva_y = posicion_mapa[1]
    ...
    def animar(self) -> None:
        """Genera la animación en el personaje"""
        lista = self.sprites_animaciones
        indice = self.indice_lista_animacion % 3
        self.setPixmap(lista[indice])
        self.indice_lista_animacion += 1
...
class FantasmaFollowerVillain(QLabel):
    def __init__(self, parent: object, posicion: tuple, id: int) -> None:
        """Inicializa la clase heredada de QLabel"""
        super().__init__(parent)
        self.posicion_inicial = posicion
    ...
    def instanciar_imagenes(self) -> None:
        ...
        a los QPixmap de cada imagen del personaje"""
        self.sprites_animaciones = []
        for imagen in IMAGENES_FANTASMA_FOLLOWERVILLAIN:
            path_imagen = os.path.join(*PATH_PERSONAJES, imagen)
            pixmap_imagen = QPixmap(path_imagen)
            self.sprites_animaciones.append(pixmap_imagen)
    ...
    def animar(self) -> None:
        """Genera la animación en el personaje"""
        lista = self.sprites_animaciones
        indice = self.indice_lista_animacion % 2
        self.setPixmap(lista[indice])
        self.indice_lista_animacion += 1
...
class Roca(QLabel):
    ...
    def instalar_personaje(self) -> None:
        """Instala el personaje en las coordenadas iniciales (x, y)"""
        self.instanciar_imagen()
        self.setGeometry(*self.posicion_inicial, *DIMENSION_SPRITES)

    def instanciar_imagen(self) -> None:
        """Instancia el QPixmap"""
        imagen = ICONOS_BLOQUES["roca"] + ".png"
        path_imagen = os.path.join(*PATH_BLOQUES, imagen)
        self.pixmap_imagen = QPixmap(path_imagen)
        self.setPixmap(self.pixmap_imagen)
...
class Estrella(QLabel):
    ...
    def instanciar_imagen(self) -> None:
        """Instancia el QPixmap"""
        imagen = ICONOS_BLOQUES["estrella"] + ".png"
        path_imagen = os.path.join(*PATH_BLOQUES, imagen)
        self.pixmap_imagen = QPixmap(path_imagen)
        self.setPixmap(self.pixmap_imagen)
...
class Fuego(QLabel):
    ...
    def instanciar_imagen(self) -> None:
        """Instancia el QPixmap"""
        imagen = ICONOS_BLOQUES["fuego"] + ".png"
        path_imagen = os.path.join(*PATH_BLOQUES, imagen)
        self.pixmap_imagen = QPixmap(path_imagen)
        self.setPixmap(self.pixmap_imagen)
...
class Pared(QLabel):
    ...
    def instanciar_imagen(self) -> None:
        """Instancia el QPixmap"""
        imagen = ICONOS_BLOQUES["pared"] + ".png"
        path_imagen = os.path.join(*PATH_BLOQUES, imagen)
        self.pixmap_imagen = QPixmap(path_imagen)
        self.setPixmap(self.pixmap_imagen)
